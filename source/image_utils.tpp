...
namespace indoor_context {
	...
	inline void Recentre(ImageF& image) {	Recentre(image, (1<<16) - 1); }
	inline void Recentre(ImageD& image) { Recentre(image, (1<<8) - 1); }

	// Transform pixels so that 0 remains at 0 and the brighest pixel is
	// white.
	inline void Rescale(ImageF& image) { Rescale(image, (1<<16) - 1); }
	inline void Rescale(ImageD& image) { Rescale(image, (1<<8) - 1); }

	// Take absolute and then apply Rescale()
	inline void RescaleAbs(ImageF& image) { RescaleAbs(image, (1<<16) - 1); }
	inline void RescaleAbs(ImageD& image) { RescaleAbs(image, (1<<8) - 1); }

	// Copy one image into another with translation and scale changes
	void CopyImageScaled(const ImageRGB<unsigned char>& image,
											 int destx,
											 int desty,
											 int destsize,
	...
	// Copy one image into another with translation and scale changes
	void CopyImageInto(const ImageRGB<unsigned char>& image,
										 int destr,
										 int destc,
										 ImageRGB<unsigned char>& canvas);
	...
	void DrawPolygon(ImageRGB<byte>& canvas,
		...
		for (int i = 0; i < N; i++) {
			DrawLine(project(poly.verts[i]), project(poly.verts[(i+1)%N]), color);
		}
	}
	...
	void DrawPolygonClipped(ImageRGB<byte>& canvas,
													...
													const PixelRGB<byte>& color) {
		for (int i = 0; i < N; i++) {
			DrawLineClipped(project(poly.verts[i]), project(poly.verts[(i+1)%N]), color);
		}
	}
