...
def read_examples(filename, sparm):
    ...
    #T_true = np.array([[0,1,0],[0,1,0],[0,1,0]])
    T_true = 1 + 2*np.eye(3)

    #return diagonal.generate_training_data(nexamples=30,
    #                                      noutliers=5,
    #                                      ntimesteps=10,
    #                                      nstates=20)
    #return diagonal.generate_trivial_data(nexamples=5,
    #                                      ntimesteps=10,
    #                                      nstates=4,
    #                                      T=T_true)
    return diagonal.generate_confounding_data(nexamples=5,
                                              ntimesteps=50,
                                              nstates=10,
                                              T=T_true)
...
def init_model(sample, model, sparm):
    ...
    gt_ftr = path.compute_path_features(sample[0][0], sample[0][1])
...
def loss(y, ybar, sparm):
    ...
    return path.compute_loss(y[0], ybar[0], path.L2)
