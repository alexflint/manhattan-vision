...
namespace indoor_context {
	...
	void LineSweeper::Compute(const PosedImage& pim,
		...
		vector<toon::Vector<3> > bounding_lines;
		...
		for (int i = 0; i < 3; i++) {
			...
			for (int j = 0; j < 3; j++) {
				if (i == j) continue;
				const Vector<3> pivot = pim.pc.GetImageVpt(i);
				const Vector<3> dir = pim.pc.GetImageVpt(j);
				...
				BOOST_FOREACH(const LineDetection& det, lines[i]) {
					...
					const Vector<3> vline1 = det.seg.start ^ dir;
					const Vector<3> vline2 = det.seg.end ^ dir;
					...
					BOOST_FOREACH(const Vector<3>& bound, bounding_lines) {
						...
						Vector<3> isct1 = vline1 ^ bound;
						Vector<3> isct2 = vline2 ^ bound;
					...
					BOOST_FOREACH(const LineDetection& block, lines[opp]) {
						...
						Vector<3> a = block.seg.start;  // will be _modified_ later
						Vector<3> b = block.seg.end;  // will be _modified_ later
					...
					// Find the closest block in-front and behind
					Vector<3> front_line, back_line;
					double front_proj = INFINITY, back_proj = -INFINITY;
					BOOST_FOREACH(const Vector<3>& block, blocks) {
						// Get intersection with the sweep line
						const Vector<3> pivot_line = block ^ pivot;
						const Vector<3> proj_pt = pivot_line ^ vline1;
	...
	void IsctGeomLabeller::Compute(const PosedImage& pim,
		...
		// Compute line sweeps
		TIMED("Sweep lines") sweeper.Compute(pim, lines);

		// Produce final label maps
		TIMED("Integrate support maps") {
			for (int i = 0; i < 3; i++) {
				int sumi = 0;
	...
	/*
	SegGeomLabeller::SegGeomLabeller() {
	}

	SegGeomLabeller::SegGeomLabeller(const LineSweepInput& input) {
		Compute(input);
	}

	void SegGeomLabeller::Compute(const LineSweepInput& inp) {
		Init(inp);
		const int nx = input->image.nx();
		const int ny = input->image.ny();

		// Compute line sweeps
		sweeper.Compute(*input);

		// Run segmenter
		segmenter.Compute(input->image);

		// Tally votes
		MatI votes(segmenter.num_segments, 3, 0);
		for (int i = 0; i < 3; i++) {
			for (int y = 0; y < ny; y++) {
				const int* segrow = segmenter.segmentation[y];
				const int* suprow = sweeper.support_maps[i][y];
				for (int x = 0; x < nx; x++) {
					votes[ segrow[x] ][i] += suprow[x];
				}
			}
		}

		// Determine consensus
		seglabels.Resize(segmenter.num_segments);
		confs.Resize(segmenter.num_segments, 0);
		for (int i = 0; i < segmenter.num_segments; i++) {
			int maxi, submaxi;
			float maxval = -1, submaxval = -1;
			for (int j = 0; j < 3; j++) {
				if (votes[i][j] > maxval) {
					submaxi = maxi;
					submaxval = maxval;
			...
			if (maxval < 1) {
				...
				confs[i] = min(1.0, maxval / (submaxval*10.0));
				if (rand() % 5 == 0) DLOG << confs[i];
			}
		}

		// Populate the orientation map
		for (int y = 0; y < ny; y++) {
			int* orientrow = orient_map[y];
			const int* segrow = segmenter.segmentation[y];
			for (int x = 0; x < nx; x++) {
				orientrow[x] = seglabels[segrow[x]];
			}
		}
	}

	void SegGeomLabeller::OutputOrientViz(const string& filename) const {
		CHECK(input);
		ImageRGB<byte> canvas;
		ImageCopy(input->image.rgb, canvas);
		DrawOrientViz(canvas);
		WriteImage(filename, canvas);
	}

	void SegGeomLabeller::OutputConfViz(const string& filename) const {
		CHECK(input);
		MatF confmap(input->image.ny(), input->image.nx());
		for (int y = 0; y < confmap.Rows(); y++) {
			float* confrow = confmap[y];
			const int* segrow = segmenter.segmentation[y];
			for (int x = 0; x < confmap.Cols(); x++) {
				confrow[x] = confs[segrow[x]];
			}
		}
		WriteMatrixImageRescaled(filename, confmap);
		}*/
}
