...
namespace indoor_context {
	...
	// Labels pixels by sweeping lines and voting on segment orientations
	/*class SegGeomLabeller : public GeomLabellerBase {
	public:
		// The line sweeper
		LineSweeper sweeper;
		// The segmenter
		...
		VecI seglabels;
		// Confidence estimates of above
		VecF confs;

		// Initialize empty
		SegGeomLabeller();
		// Initialize and compute
		SegGeomLabeller(const LineSweepInput& input);
		// Compute plane orientations
		void Compute(const LineSweepInput& input);
		// Produce a vizualization of the orientations
		void OutputOrientViz(const string& filename) const;
		// Vizualize the orientation confidences
		void OutputConfViz(const string& filename) const;
		};*/
}
