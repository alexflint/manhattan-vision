...
namespace indoor_context {
	...
	public:
		...
		/*// Homography from image plane to plane-at-infinity
		toon::Matrix<3> vpt_homog;
		// Homography from plane-at-infinity to image plane
		toon::Matrix<3> vpt_homog_inv;
		// Homography from image plane to plane-at-infinity for lines
		toon::Matrix<3> vpt_homog_dual;
		// Homography from plane-at-infinity to image plane for lines
	...
	public:
		...
		void LoadXml(const string& xml_file);
		// Load a PTAM v1 the map as specified by the given GVar
		/*void LoadMapDump(const string& dirname,
		                 const string& impat,
