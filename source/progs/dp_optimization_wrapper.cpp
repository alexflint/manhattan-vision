...
void PackFeatures(const vector<int>& aux_ids,
	...
	PackMatrix(joint.mono_gen.payoffs.wall_scores[0],
						 *precomputed.mutable_mono0());
	PackMatrix(joint.mono_gen.payoffs.wall_scores[1],
						 *precomputed.mutable_mono1());

	PackMatrix(joint.point_cloud_gen.agreement_payoffs,
						 *precomputed.mutable_agreement());
	PackMatrix(joint.point_cloud_gen.occlusion_payoffs,
						 *precomputed.mutable_occlusion());
	...
	for (int i = 0; i < aux_ids.size(); i++) {
		...
		PackMatrix(joint.stereo_gens[i].payoffs,
							 *precomputed.add_stereo());
	}
}

void UnpackFeatures(const proto::PayoffFeatures& precomputed,
										PayoffFeatures& features) {
	UnpackMatrix(precomputed.mono0(),
							 features.mono_payoffs.wall_scores[0]);
	UnpackMatrix(precomputed.mono1(),
							 features.mono_payoffs.wall_scores[1]);
	UnpackMatrix(precomputed.agreement(),
							 features.agreement_payoffs);
	UnpackMatrix(precomputed.occlusion(),
							 features.occlusion_payoffs);

	features.stereo_payoffs.clear();
	CHECK_EQ(precomputed.stereo_aux_ids_size(),
					 precomputed.stereo_size());
	for (int i = 0; i < precomputed.stereo_size(); i++) {
		features.stereo_payoffs.push_back(new MatF);
		UnpackMatrix(precomputed.stereo(i),
								 features.stereo_payoffs.back());
	}
}
...
int main(int argc, char **argv) {
    ...
    ("mono_weight", po::value<float>()->required(),
		 "weight for mono payoffs")
    ("stereo_weight", po::value<float>()->required(),
		 "weight for stereo payoffs")
    ("3d_agreement_weight", po::value<float>()->required(),
		 "weight for 3D agreement payoffs")
    ("3d_occlusion_weight", po::value<float>()->required(),
		 "weight for 3D occlusion payoffs")
    ("corner_penalty", po::value<float>()->required(),
		 "per-corner penalty")
    ...
    ("sequence", po::value<vector<string> >()->required(),
		 "sequences to evaluate")
		("frame_stride", po::value<int>()->default_value(1),
		 "number of frames per evaluation")

		("store_payoffs", po::value<string>(), 
		 "write payoff matrices to this dir (skips evaluation)")
		("load_payoffs", po::value<string>(), 
		 "load payoff matrices from this dir (as generated by --store_payoffs)")
		("evaluate",
		 "if provided, forces evaluation even when --store_payoffs is specified")

    ("concurrency", po::value<int>()->default_value(1),
		 "number of independent threads -- not working yet")

		("visualization_dir", po::value<string>()->default_value("out"),
