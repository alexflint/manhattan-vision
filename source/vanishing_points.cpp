	...
	void VanishingPoints::DrawVanPointViz(ImageRGB<byte>& canvas,
		...
		COUNTED_FOREACH (int i, const LineDetection& det, lines.detections) {
			if (det.axis == -1) {
				...
				// TODO: ClipToImage(a, b, canvas.GetSize())
				CvSize sz = cvSize(canvas.GetWidth(), canvas.GetHeight());
				CvPoint cva = cvPoint(a[0], a[1]);
				CvPoint cvb = cvPoint(b[0], b[1]);
				cvClipLine(sz, &cva, &cvb);
				Vector<2> cl_a = makeVector(cva.x, cva.y);
				Vector<2> cl_b = makeVector(cvb.x, cvb.y);
				DrawLine(canvas, cl_a, cl_b, line_colors[i], *gvVanLineAlpha);
			}
		}
