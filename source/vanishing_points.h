...
namespace indoor_context {
	...
	public:
		...
		vector<toon::Vector<3> > line_eqns;
		...
		// vanishing point.
		static double GetLogLik(const toon::Vector<3>& vpt,
														const toon::Vector<3>& line);
		// Fit the least-squares intersection point using SVD
		toon::Vector<3> FitIsct(const vector<LineDetection>& segments,
														const toon::Vector<-1>& weights = toon::Zeros) const;
		// Fit a robust intersection point using RANSAC
		toon::Vector<3> FitIsctRansac(const vector<LineDetection>& segments,
																	const toon::Vector<-1>& weights = toon::Zeros) const;
