...
bool Widget3D::HitTest(const Vector<2>& mouse) const {
	return false;
}

Widget3D* Widget3D::GetWidgetAt(const Vector<2>& mouse) {
...
void Widget3D::NotifyMouseDown(int button, const Vector<2>& mouse) {
	if (selectable()) {
		SetSelected(true);
	}
	MouseDown.fire();
...
void Widget3D::NotifyMouseUp(int button, const Vector<2>& mouse) {
	MouseUp.fire();
}

void Widget3D::NotifyMouseMove(const Vector<2>& mouse) {
	MouseMove.fire();
}

void Widget3D::NotifyMouseDrag(int button, const Vector<2>& mouse) {
	MouseDrag.fire();
}

void Widget3D::NotifyClick(int button, const Vector<2>& mouse) {
	OnClick(button, mouse);
	Click.fire();
}

void Widget3D::NotifyDoubleClick(int button, const Vector<2>& mouse) {
...
QuadWidget::QuadWidget(const toon::Vector<3>& aa,
                       const toon::Vector<3>& bb,
                       const toon::Vector<3>& cc,
                       const toon::Vector<3>& dd)
: a(aa), b(bb), c(cc), d(dd), color(255,255,0), border_width(1.2) {
}
...
QuadWidget::QuadWidget(const Matrix<3,2>& m,
                       ...
                       float top,
                       float bottom)
: a(m*makeVector(left,top)),
  b(m*makeVector(right,top)),
  c(m*makeVector(right,bottom)),
  d(m*makeVector(left,bottom)),
  color(255,255,0),
  border_width(1.2) {
}
...
LineWidget& LineWidgetBloc::AddNewMember(const Vector<3>& start,
                                         const Vector<3>& end,
...
LineWidget::LineWidget(const Vector<3>& start,
                       const Vector<3>& end,
                       float w,
...
bool LineWidget::HitTest(const Vector<2>& mouse) const {
	if (selectable()) {
		Vector<2> sa = project(screen_line.start);
		Vector<2> sb = project(screen_line.end);
		Vector<2> smid = (sa+sb)/2.0;
		Vector<2> sline = makeVector(sa[1]-sb[1], sb[0]-sa[0]) / norm(sb-sa);
